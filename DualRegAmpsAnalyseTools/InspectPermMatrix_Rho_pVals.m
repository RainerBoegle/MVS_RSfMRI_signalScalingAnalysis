...
if(isstruct(MVSscalingTest.PermTest))
    if(isfield(MVSscalingTest.PermTest,'PermMatrix_Rho_pVals'))
        ...
        ax1 = subplot(2,2,1); hold on;
                boxplot(MVSscalingTest.PermTest.PermMatrix_Rho_pVals(:,:,1,1),'notch','on','labels',MVSscalingTest.PermTest.CovarsStruct.CovarName); title('Pearson correlation of covariates with PERMUTED covariates'); ylabel('Rho'); ylim([-1.01; 1.01]); 
                plot(x,0.3.*ones(size(x)),'g--'); plot(x,-0.3.*ones(size(x)),'g--'); plot(x,0.5.*ones(size(x)),'r--'); plot(x,-0.5.*ones(size(x)),'r--');
        ax2 = subplot(2,2,2); hold on;
                boxplot(MVSscalingTest.PermTest.PermMatrix_Rho_pVals(:,:,1,2),'notch','on','labels',MVSscalingTest.PermTest.CovarsStruct.CovarName); title('Spearman (RANK) correlation of covariates with PERMUTED covariates'); ylabel('Rho'); ylim([-1.01; 1.01]);
                plot(x,0.3.*ones(size(x)),'g--'); plot(x,-0.3.*ones(size(x)),'g--'); plot(x,0.5.*ones(size(x)),'r--'); plot(x,-0.5.*ones(size(x)),'r--');
        ax3 = subplot(2,2,3); hold on;
                boxplot(MVSscalingTest.PermTest.PermMatrix_Rho_pVals(:,:,2,1),'notch','on','labels',MVSscalingTest.PermTest.CovarsStruct.CovarName); ylabel('P-Vals'); ylim([-0.01; 1.01]);
                plot(x,0.05.*ones(size(x)),'r--'); plot(x,0.1.*ones(size(x)),'g--');
        ax4 = subplot(2,2,4); hold on;
                boxplot(MVSscalingTest.PermTest.PermMatrix_Rho_pVals(:,:,2,2),'notch','on','labels',MVSscalingTest.PermTest.CovarsStruct.CovarName); ylabel('P-Vals'); ylim([-0.01; 1.01]);
                plot(x,0.05.*ones(size(x)),'r--'); plot(x,0.1.*ones(size(x)),'g--');
