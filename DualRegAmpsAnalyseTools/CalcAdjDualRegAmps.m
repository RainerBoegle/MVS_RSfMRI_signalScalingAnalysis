function [AdjAmps,OrgAmps,AdjValues,Mask3D,DualRegAmpsFilePath,AdjustDataPaths,MaskPath,PrefixStr] = CalcAdjDualRegAmps(DualRegAmpsFilePath,AdjustDataPaths,MaskPath,PrefixStr)
...
try
    if(iscellstr(AdjustDataPaths))
        ...
        if(ischar(AdjustDataPaths))%check 4D-NIFTI, otherwise error
            ...
            if(length(AdjValuesNII)~=OrgAmpsNII.dat.dim(4))
                ...
                disp('Collecting AdjustValues data from 3D-NIFTI objects/files...');
                AdjValues4D = zeros([AdjValuesNII(1).dat.dim,length(AdjValuesNII)]);
...
%% output as NIFTI
%Vout(size(OrgAmps4D,4)).descrip = ['Adjusted DualRegAmplitudes "',PrefixStr,'"']; %init struct (preallocate)
...
for IndOut = 1:size(OrgAmps4D,4)  
    %need to do this due to awful way spm treats 4D-files
    Vtmp = V_OrgAmps(IndOut);
    %make adjustments to volume struct
    if(isfield(Vtmp,'private'))
        Vtmp = rmfield(Vtmp,'private');
    end
    if(Vtmp.dt(1)<16)
        Vtmp.dt(1) = 16; %safe in terms of resolution/bit depth.
    end
    Vtmp.n(1)    = IndOut;
    Vtmp.fname   = [BaseDirOrgAmps,filesep,PrefixStr,fNameOrgAmps,ext]; %add prefix string
    Vtmp.descrip = ['Adjusted DualRegAmplitudes "',PrefixStr,'"'];
    
    %assign struct (%need to do this due to awful way spm treats 4D-files)
    Vout(IndOut) = Vtmp;
