function [OutputStruct]=ExpandAtlasOverlap(Overlap_MaskEffectsOfMRI_AAL_Path,Mask_EffectsOfMRI_Path,varargin)
...
%
%Usage:
%      [OutputStruct]=ExpandAtlasOverlap(Overlap_MaskEffectsOfMRI_AAL_Path,Mask_EffectsOfMRI_Path,ShowWarnings,MaxIterFactor,NCyclesNoChangeMax); Default: Don't show warnings and Maximum Iteration Factor == 10 and NCyclesNoChangeMax == 20
%      [OutputStruct]=ExpandAtlasOverlap(Overlap_MaskEffectsOfMRI_AAL_Path,Mask_EffectsOfMRI_Path,1);        %Show warnings & Iterate  10-times-NVoxels to assign if not 20 cycles are unchanged.  
%      [OutputStruct]=ExpandAtlasOverlap(Overlap_MaskEffectsOfMRI_AAL_Path,Mask_EffectsOfMRI_Path,1,100);    %Show warnings & Iterate 100-times-NVoxels to assign if not 20 cycles are unchanged. 
%      [OutputStruct]=ExpandAtlasOverlap(Overlap_MaskEffectsOfMRI_AAL_Path,Mask_EffectsOfMRI_Path,1,100,10); %Show warnings & Iterate 100-times-NVoxels to assign if not 10 cycles are unchanged. 
%
%
...
%% get Overlap_MaskEffectsOfMRI_AAL NIFTI
if(~exist(Overlap_MaskEffectsOfMRI_AAL_Path))
    [basedir,fname,ext] = fileparts(Overlap_MaskEffectsOfMRI_AAL_Path); clear basedir ext
    Overlap_MaskEffectsOfMRI_AAL_Path = spm_select(1,'image',['Select NIFTI Overlap with AAL atlas ["',fname,'???"]...']); clear fname
end
[OutputDir,OrgFName] = fileparts(Overlap_MaskEffectsOfMRI_AAL_Path);
...
if(~exist(Mask_EffectsOfMRI_Path))
...
load([FunctionDir,filesep,'AAL_Atlas',filesep,'AAL_LabelsOnly.mat']);
...
switch(choice_search)
    case 'NearestNeighbors'
        Vo.fname = [OutputDir,filesep,'Expanded_NN_',OrgFName,'.nii'];
        OutName  = ['Expanded_NN_',OrgFName,'.nii'];
    case {'ManualNeighbors','IterateOrders'}
        Vo.fname = [OutputDir,filesep,'Expanded_',choice_search,'_',num2str(max(NOrd)),'N_',OrgFName,'.nii'];
...
disp(' ');
disp(['Results have been written out as "',OutName,'" to "',OutputDir,'"']);
if(~isempty(CurrentIndices)) %some are missing so let's show them separately
    ...
    switch(choice_search)
        case 'NearestNeighbors'
            VoNotAssigned.fname = [OutputDir,filesep,'StillMissingVoxels_Expanded_NN_',OrgFName,'.nii'];
            OutName  = ['StillMissingVoxels_Expanded_NN_',OrgFName,'.nii'];
        case {'ManualNeighbors','IterateOrders'}
            VoNotAssigned.fname = [OutputDir,filesep,'StillMissingVoxels_Expanded_',choice_search,'_',num2str(max(NOrd(:))),'N_',OrgFName,'.nii'];
            OutName  = ['StillMissingVoxels_Expanded_',choice_search,'_',num2str(max(NOrd(:))),'N_',OrgFName,'.nii'];
    end    
    if(VoNotAssigned.dt(1)<16)
        VoNotAssigned.dt(1) = 16; %not necessary but save
    end
    VoNotAssigned = spm_write_vol(VoNotAssigned, MissingVoxels);
    disp(['The voxels that are still missing have been written out as "',OutName,'" to "',OutputDir,'"']);
end
...
%% write out the data
[OutputDir,OrgFName,Ext] = fileparts(Vo.fname);

VoBasic = spm_vol(Overlap_MaskEffectsOfMRI_AAL_Path);
VoBasic.fname = [OutputDir,filesep,'CombineBasic_',OrgFName,'.nii'];
...
VoGeneral.fname = [OutputDir,filesep,'CombineGeneral_',OrgFName,'.nii'];
...
save([OutputDir,filesep,'AAL_Labels.mat'],'Labels');
